...
if($_POST['aId']){
	...
	$getFieldList = "SELECT fieldList, sortString, dirString, emE, gpE FROM hon_server.ladderFieldsRanked WHERE account_id = '".mysql_real_escape_string($_POST['aId'])."' LIMIT 1";
	if($doGetFieldList = mysql_query($getFieldList,$l1)){
		if(mysql_num_rows($doGetFieldList) > 0){
			$fRow = mysql_fetch_assoc($doGetFieldList);
			
			if($_POST['s']){
				$update = "UPDATE hon_server.ladderFieldsRanked SET sortString = '".mysql_real_escape_string($_POST['s'])."' WHERE account_id = '".mysql_real_escape_string($_POST['aId'])."' LIMIT 1";
			...
			if($_POST['d']){
				$update = "UPDATE hon_server.ladderFieldsRanked SET dirString = '".mysql_real_escape_string($_POST['d'])."' WHERE account_id = '".mysql_real_escape_string($_POST['aId'])."' LIMIT 1";
				if(!$doUpdate = mysql_query($update,$l1)){
					@mysql_close($l1);
					print serialize(array(0 => "failure", 1 => 1));
					exit;
			...
			if($_POST['emE']){
				$update = "UPDATE hon_server.ladderFieldsRanked SET emE = '".mysql_real_escape_string($_POST['emE'])."' WHERE account_id = '".mysql_real_escape_string($_POST['aId'])."' LIMIT 1";
				if(!$doUpdate = mysql_query($update,$l1)){
					@mysql_close($l1);
					print serialize(array(0 => "failure", 1 => 1));
					exit;
			...
			if($_POST['gpE']){
				$update = "UPDATE hon_server.ladderFieldsRanked SET dirString = '".mysql_real_escape_string($_POST['gpE'])."' WHERE account_id = '".mysql_real_escape_string($_POST['aId'])."' LIMIT 1";
				if(!$doUpdate = mysql_query($update,$l1)){
					@mysql_close($l1);
					print serialize(array(0 => "failure", 1 => 1));
					exit;
			...
			if($_POST['a']){
				...
				$update = "UPDATE hon_server.ladderFieldsRanked SET fieldList = '".mysql_real_escape_string($fieldList)."' WHERE account_id = '".mysql_real_escape_string($_POST['aId'])."' LIMIT 1";
				if(!$doUpdate = mysql_query($update,$l1)){
					@mysql_close($l1);
					print serialize(array(0 => "failure", 1 => 1));
					exit;
			...
			$fieldList = $defaultFieldList;
			$insert = "INSERT INTO hon_server.ladderFieldsRanked (account_id, fieldList) VALUES ('".mysql_real_escape_string($_POST['aId'])."', '".mysql_real_escape_string($fieldList)."')";
...
@mysql_close($l1);
include('clientLadderClassRanked.php');
/*
a(action) = 	r(remove)
				a(add)
				x(reset)
