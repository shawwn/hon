...
class ClientController{
    ...
    function get_field_stats(& $conn){
        ...
        // If there were any additional fields passed in the request then transfer those variables over to the output array
        if (isset($_REQUEST['tmmtype'])) $conn->ret_array['tmmtype'] = $_REQUEST['tmmtype'];
        if (isset($_REQUEST['gametype'])) $conn->ret_array['gametype'] = $_REQUEST['gametype'];
        if (isset($_REQUEST['teamsize'])) $conn->ret_array['teamsize'] = $_REQUEST['teamsize'];
        if (isset($_REQUEST['gamemodes'])) $conn->ret_array['gamemodes'] = $_REQUEST['gamemodes'];
        if (isset($_REQUEST['mapnames'])) $conn->ret_array['mapnames'] = $_REQUEST['mapnames'];
        if (isset($_REQUEST['regions'])) $conn->ret_array['regions'] = $_REQUEST['regions'];
        if (isset($_REQUEST['leaderaccountid'])) $conn->ret_array['leaderaccountid'] = $_REQUEST['leaderaccountid'];
