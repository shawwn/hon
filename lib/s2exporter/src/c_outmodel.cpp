...
COutModel::COutModel(INode *pNode, TimeValue time) :
m_bIsValid(false),
...
tstring	COutModel::WriteFile(const tstring &sFilename)
{
	...
	if (stream == NULL)
	{
		g_S2ModelExporter.MessageBox("Couldn't create .model file", "File Error");
		return _T("");
	}
	...
	for (vector<COutBone>::iterator boneit(m_vBones.begin()); boneit != m_vBones.end(); ++boneit)
	{
		...
		strncpy(boneblock.szBoneName, boneit->GetName().c_str(), boneblock.cNameLen);
	...
	for (vector<COutMesh>::iterator meshit(m_vMeshes.begin()); meshit != m_vMeshes.end(); ++meshit)
	{
		...
		strncpy(meshblock.szNameBuffer, meshit->GetName().c_str(), meshblock.cNameLen);
		meshblock.szNameBuffer[meshblock.cNameLen] = 0;

		meshblock.cShaderNameLen = MIN(meshit->GetDefaultShaderName().size(), size_t(255));
		strncpy(&meshblock.szNameBuffer[meshblock.cNameLen + 1], meshit->GetDefaultShaderName().c_str(), meshblock.cShaderNameLen);
	...
	for (vector<CConvexPolyhedron>::iterator surfit(m_vSurfs.begin()); surfit != m_vSurfs.end(); ++surfit)
	{
		...
		// Write planes
		for (int p = 0; p < surfit->GetNumPlanes(); ++p)
			fwrite(&surfit->GetPlane(p), sizeof(CPlane), 1, stream);

		// Write points
		for (int p = 0; p < surfit->GetNumPoints(); ++p)
			fwrite(&surfit->GetPoint(p), sizeof(CVec3f), 1, stream);

		// Write edges
		for (int p = 0; p < surfit->GetNumEdges(); ++p)
