...
void RemapIndices(const PrimitiveGroup* in_primGroups, const unsigned short numGroups,
				  const unsigned short numVerts, unsigned int** remappedIndices)
{
	(*remappedIndices) = new unsigned int[numVerts];
	memset((*remappedIndices), -1, sizeof(unsigned int)*numVerts);
	...
	for(int i = 0; i < numGroups; i++)
	{
		...
		{
			int cachedIndex = (*remappedIndices)[in_primGroups[i].indices[j]];
			if(cachedIndex == -1) //we haven't seen this index before
			{
				//point to "last" vertex in VB
				(*remappedIndices)[in_primGroups[i].indices[j]] = indexCtr++;
			}
		}
	}
}