...
/*====================
  MD5String
  ====================*/
string	MD5String(const string &sSource)
{
	...
	MD5Update(mdContext, (byte*)sSource.c_str(), INT_SIZE(sSource.length() * sizeof(char)));
	MD5Final(mdContext);

	string sReturn;
	for (int i(0); i < 16; ++i)
		sReturn += XtoS(ushort(mdContext.digest[i]), FMT_NOPREFIX | FMT_PADZERO | FMT_LOWERCASE, 2, 16);

	return sReturn;
}

string	MD5String(const wstring &sSource)
{
	SMD5Context mdContext;
	...
	MD5Update(mdContext, (byte*)sSource.c_str(), INT_SIZE(sSource.length() * sizeof(wchar_t)));
	MD5Final(mdContext);
...
/*====================
  MD5WString
  ====================*/
wstring	MD5WString(const string &sSource)
{
	SMD5Context mdContext;
	...
	wstring sReturn;
	for (int i(0); i < 16; ++i)
		sReturn += XtoW(ushort(mdContext.digest[i]), FMT_NOPREFIX | FMT_PADZERO | FMT_LOWERCASE, 2, 16);

	return sReturn;
}

wstring	MD5WString(const wstring &sSource)
{
	SMD5Context mdContext;
