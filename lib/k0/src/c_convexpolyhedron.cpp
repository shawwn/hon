...
{
	for (vector<CPlane>::iterator it(m_vPlanes.begin()); it != m_vPlanes.end(); ++it)
	{
...
bool	CConvexPolyhedron::AddAxis(const CVec3f &v3Axis)
{
	...
	if (fabs(1.0f - v3Axis.Length()) > 0.1f) // Check for malformed normal
		return false;

	// Skip duplicated axes
	vector<SExtents>::iterator findit;
	for (findit = m_vExtents.begin(); findit != m_vExtents.end(); ++findit)
	{
		if (findit->v3Axis == v3Axis || findit->v3Axis == -v3Axis)
			break;
	}

	if (findit != m_vExtents.end())
		return false;
