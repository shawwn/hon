...
void	FormatFloat(double d, int flags, size_t width, int precision, string &sStr)
{
	...
	// Create padding
	char padchar(' ');
	int padcount(int(width) - ((sign == 0) ? 0 : 1) - MAX(INT_SIZE(sStr.length()), 1u));
	if (flags & FMT_PADZERO && !(flags & FMT_ALIGNLEFT))
		padchar = '0';
	if (padcount < 0)
		padcount = 0;
...
/*====================
  XtoA
 ====================*/
string	XtoS(short s, int flags, int width, int base)
{
	char sign(0);
	if (s < 0)
	{
	...
	string sReturn;
	FormatInt(s, flags, width, base, sign, sReturn);
	return sReturn;
}

wstring	XtoW(short s, int flags, int width, int base)
{
	wchar_t sign(0);
	if (s < 0)
	{
...
string	XtoS(int i, int flags, int width, int base)
{
	char sign(0);
	if (i < 0)
	{
	...
	string sReturn;
	FormatInt(i, flags, width, base, sign, sReturn);
	return sReturn;
}

wstring	XtoW(int i, int flags, int width, int base)
{
	wchar_t sign(0);
	if (i < 0)
	{
...
string	XtoS(long l, int flags, int width, int base)
{
	char sign(0);
	if (l < 0)
	{
	...
	string sReturn;
	FormatInt(l, flags, width, base, sign, sReturn);
	return sReturn;
}

wstring	XtoW(long l, int flags, int width, int base)
{
	wchar_t sign(0);
	if (l < 0)
	{
...
string	XtoS(LONGLONG ll, int flags, int width, int base)
{
	char sign(0);
	if (ll < 0)
	{
	...
	string sReturn;
	FormatInt(ll, flags, width, base, sign, sReturn);
	return sReturn;
}

wstring	XtoW(LONGLONG ll, int flags, int width, int base)
{
	wchar_t sign(0);
	if (ll < 0)
	{
...
string	XtoS(unsigned short us, int flags, int width, int base)
{
	string sReturn;
	FormatInt(us, flags, width, base, 0, sReturn);
	return sReturn;
}

wstring	XtoW(unsigned short us, int flags, int width, int base)
{
	wstring sReturn;
	FormatInt(us, flags, width, base, 0, sReturn);
	return sReturn;
...
string	XtoS(unsigned int ui, int flags, int width, int base)
{
	string sReturn;
...
wstring	XtoW(unsigned int ui, int flags, int width, int base)
{
	wstring sReturn;
	FormatInt(ui, flags, width, base, 0, sReturn);
	return sReturn;
...
string	XtoS(unsigned long ul, int flags, int width, int base)
{
	string sReturn;
	FormatInt(ul, flags, width, base, 0, sReturn);
	return sReturn;
}

wstring	XtoW(unsigned long ul, int flags, int width, int base)
{
	wstring sReturn;
	FormatInt(ul, flags, width, base, 0, sReturn);
	return sReturn;
...
string	XtoS(ULONGLONG ull, int flags, int width, int base)
{
	string sReturn;
	FormatInt(ull, flags, width, base, 0, sReturn);
	return sReturn;
}

wstring	XtoW(ULONGLONG ull, int flags, int width, int base)
{
	wstring sReturn;
	FormatInt(ull, flags, width, base, 0, sReturn);
	return sReturn;
...
string	XtoS(float f, int flags, int width, int precision)
{
	string sReturn;
	FormatFloat(f, flags, width, precision, sReturn);
	return sReturn;
}

wstring	XtoW(float f, int flags, int width, int precision)
{
	wstring sReturn;
	FormatFloat(f, flags, width, precision, sReturn);
	return sReturn;
...
string	XtoS(double d, int flags, int width, int precision)
{
	string sReturn;
	FormatFloat(d, flags, width, precision, sReturn);
	return sReturn;
}

wstring	XtoW(double d, int flags, int width, int precision)
{
	wstring sReturn;
	FormatFloat(d, flags, width, precision, sReturn);
	return sReturn;
...
string	XtoS(bool b, bool bNum)
{
	return (bNum ? (b ? "1" : "0") : (b ? "true" : "false"));
}

wstring	XtoW(bool b, bool bNum)
{
	return (bNum ? (b ? L"1" : L"0") : (b ? L"true" : L"false"));
}
...
string	XtoS(const CVec4f &v4, int flags, int width, int precision)
{
	...
	string str(sX + SPACE + sY + SPACE + sZ + SPACE + sW);
...
wstring	XtoW(const CVec4f &v4, int flags, int width, int precision)
{
	...
	wstring str(sX + WSPACE + sY + WSPACE + sZ + WSPACE + sW);
...
string	XtoS(const void *p, bool bLower)
{
	string sReturn;
	FormatInt(reinterpret_cast<size_t>(p), FMT_PADZERO, sizeof(size_t) * 2, 16, '\0', sReturn);
	return sReturn;
}

wstring	XtoW(const void *p, bool bLower)
{
	wstring sReturn;
	FormatInt(reinterpret_cast<size_t>(p), FMT_PADZERO, sizeof(size_t) * 2, 16, L'\0', sReturn);
	return sReturn;
}
...
{
	if (CompareNoCase(s, L"true") == 0)
		return true;
	else
		return (WTOF(s.c_str()) != 0);
}
...
int		HexAtoI(const tstring &s)
{
	...
	int iResult(0);
	int iMultiple(1);
	for (tstring::const_reverse_iterator cit(s.rbegin()); cit != s.rend(); ++cit)
	{
		...
		{
			cerr << _T("HexAtoI() - Invalid value: ") << s << endl;
			return 0;
		}
