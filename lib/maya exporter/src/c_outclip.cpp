...
void	COutClip::WriteFloatKeys(int boneindex, SFloatKeys *keys, int keytype, const tstring &sBoneName, FILE *stream)
{
	...
	strncpy_s(keyblock.szBoneName, 256, sBoneName.c_str(), keyblock.cNameLen);
...
/*====================
  COutClip::WriteFile
  ====================*/
string	COutClip::WriteFile(const tstring &sFilename)
{
	FILE* stream(NULL);
	if (!fopen_s(&stream, sFilename.c_str(), "wb"))
	{
		g_ModelExporter.Error("Couldn't create .clip file");
		return "";
	}
	...
	int iNumframes(g_ModelExporter.GetNumFrames());
