...
VS_OUTPUT VS( VS_INPUT In )
{
	...
	Out.Last.z = length(vPositionWorld);
#endif
...
#elif (LIGHTING_QUALITY == 1)
	float3 vCamDirection = -normalize(vPositionWorld);
	float3 vLight = vSunPosition.xyz;
...
#elif (LIGHTING_QUALITY == 2)
	float3 vCamDirection = -normalize(vSunPositionWorld);
	float3 vLight = vSunPosition.xyz;		
