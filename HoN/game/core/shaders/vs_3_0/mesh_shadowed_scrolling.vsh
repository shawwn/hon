...
struct VS_OUTPUT
{
	...
	float CamDistance : TEXCOORDX;
		#include "../common/inc_texcoord.h"
#endif
#if (FOG_OF_WAR == 1)
	float2 TexcoordFogofWar : TEXCOORDX;
		#include "../common/inc_texcoord.h"
#endif
};
...
VS_OUTPUT VS( VS_INPUT In )
{
	...
	{
		mAxis	+= float3x3(vBones[In.BoneIndex[i] * 3 + 0].xyz,
		                    vBones[In.BoneIndex[i] * 3 + 1].xyz,
		                    vBones[In.BoneIndex[i] * 3 + 2].xyz) * In.BoneWeight[i];
	
		vPos	+= float3(vBones[In.BoneIndex[i] * 3 + 0].w, vBones[In.BoneIndex[i] * 3 + 1].w, vBones[In.BoneIndex[i] * 3 + 2].w) * In.BoneWeight[i];
	}
	
	vPosition = mul(In.Position, mAxis) + vPos;
	vNormal = mul(In.Normal, mAxis);
	vTangent = mul(In.Tangent, mAxis);
	...
	Out.Color0        = vColor;
	Out.Texcoord0     = In.Texcoord0 + float2(0.f, fSpeed * fTime);

#if (LIGHTING_QUALITY == 0 || FALLOFF_QUALITY == 0)
	Out.PositionView  = vPositionView;
#endif
#if (FALLOFF_QUALITY == 1 && (FOG_TYPE != 0 || SHADOWS == 1))
	Out.CamDistance = length(vPositionView);
#endif
...
#if (FOG_QUALITY == 1 && FOG_TYPE != 0)
	Out.Fog = Fog(vPositionView);
#endif

#if (FOG_OF_WAR == 1)
	Out.TexcoordFogofWar = mul(float4(vPosition,1.f),mWorld).xy / float2(fWorldWidth, fWorldHeight);
#endif
