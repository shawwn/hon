...
void main()
{
	...
	{
		int iIndex = int(vBlendIndex[i]);
	
		mBlend += vBones[iIndex] * vBoneWeight[i];
	}
	#else
	mat4 mBlend;
	mBlend[0] = vec4(0.0, 0.0, 0.0, 0.0);
	mBlend[1] = vec4(0.0, 0.0, 0.0, 0.0);
	mBlend[2] = vec4(0.0, 0.0, 0.0, 0.0);
	mBlend[3] = vec4(0.0, 0.0, 0.0, 1.0);
	for (int i = 0; i < NUM_BONE_WEIGHTS; ++i)
	{
		int iIndex = int(vBlendIndex[i]) * 3;
	
		mBlend[0] += vBones3[iIndex + 0] * vBoneWeight[i];
		mBlend[1] += vBones3[iIndex + 1] * vBoneWeight[i];
		mBlend[2] += vBones3[iIndex + 2] * vBoneWeight[i];
	}
	#endif
	...
	vec3 vWVBinormal = cross(vWVTangent, vWVNormal) * a_vTangent.w;
